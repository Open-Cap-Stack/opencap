...
describe('Activity Controller', () => {
  ...
  it('Successfully creates activity log', async () => {
    const documentWithoutRequiredField = new Document({
      documentId: 'unique_id_2',
    });
    let err;
    try {
      const savedDocumentWithoutRequiredField =
        await documentWithoutRequiredField.save();
    } catch (error) {
      err = error;
    }
  ...
  it('Should respond with the newly created activity log', async () => {
    ...
    const activityLogStub = sinon
      .stub(ActivityLog.prototype, 'save')
      .resolves(savedActivityLog);
