#!/bin/bash
# OpenCap SPV Endpoint Test Runner
# Follows Semantic Seed Coding Standards v2.0 and OpenCap Standards v1.0
# Focused test script that runs only the SPV endpoint tests
...
# Run only the API tests for specific routes we've been updating
echo "Running SPV and related endpoint tests..."
npx jest \
  --testTimeout=$TEST_TIMEOUT \
  --forceExit \
  --detectOpenHandles \
  routes/spvRoutes \
  routes/complianceCheckRoutes \
  "$@"
