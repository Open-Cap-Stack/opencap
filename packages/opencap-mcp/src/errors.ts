...
export function formatMcpError(err: unknown): string {
  ...
  if (status === 401 || status === 403) {
    return (
      'Access denied. Check your API key and ensure companyId in the request matches your account. ' +
  ...
  if (status === 404) {
    return (
      'Record not found. Make sure you are using the domain ID (e.g. `safeId`, `row_id`), ' +
  ...
  if (status === 500) {
    return (
      'The server could not save the record. This may be a temporary issue, or a referenced ID ' +
