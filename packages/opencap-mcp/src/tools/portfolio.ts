...
export const portfolioTools: ToolDefinition[] = [
  {
    ...
    description:
      'Summarize an investor\'s portfolio across all companies they hold shares in. ' +
    ...
    handler: async (input, client) => {
      ...
      const stakeholders = (listData.stakeholders ?? listData) as Array<{
        row_id: string;
        name: string;
        companyId: string;
        sharesOwned?: number;
      ...
      const positions = stakeholders.filter(
        (sh) => sh.row_id === input.investorId || (sh as unknown as Record<string, string>)['investorId'] === input.investorId
      );
...

  {
    ...
    description:
      'Calculate dilution impact across all portfolio companies for a given investor. ' +
    ...
    handler: async (input, client) => {
      ...
      const dilutionData = await Promise.all(
        companyIds.map(async (companyId) => {
          try {
        ...
        content: [
          {
            type: 'text',
            text: JSON.stringify(
              { investorId: input.investorId, scenario: input.scenario ?? 'current', dilution: dilutionData },
              null,
              2
            ),
          },
...

  {
    ...
    description:
      'Full investor dashboard across all portfolio holdings. ' +
