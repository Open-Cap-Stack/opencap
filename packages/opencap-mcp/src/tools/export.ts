...
export const exportTools: ToolDefinition[] = [
  {
    ...
    description:
      'Export the full cap table as CSV or JSON data. Combines cap table summary, stakeholders, ' +
    ...
    handler: async (input, client) => {
      ...
      if (input.format === 'csv') {
        const stk = capTable.stakeholders as Record<string, unknown>[];
        if (stk.length === 0) {
          return {
            content: [
              {
