...
export default function ShareClassesPage() {
  ...
  const createMut = useMutation({ mutationFn: (d) => shareClassService.createShareClass(d), onSuccess: () => { setMutationError(null); qc.invalidateQueries({ queryKey: ['shareClasses'] }); setModal({ open: false, editing: null }); }, onError: (err) => { setMutationError(err.response?.data?.message || 'Failed to create share class'); } });
  const updateMut = useMutation({ mutationFn: ({ id, ...d }) => shareClassService.updateShareClass(id, d), onSuccess: () => { setMutationError(null); qc.invalidateQueries({ queryKey: ['shareClasses'] }); setModal({ open: false, editing: null }); }, onError: (err) => { setMutationError(err.response?.data?.message || 'Failed to update share class'); } });
  const deleteMut = useMutation({ mutationFn: (id) => shareClassService.deleteShareClass(id), onSuccess: () => { qc.invalidateQueries({ queryKey: ['shareClasses'] }); setDeleteId(null); }, onError: (err) => { alert(err.response?.data?.message || 'Failed to delete share class'); } });

  const openCreate = () => { setForm(emptyForm); setMutationError(null); setModal({ open: true, editing: null }); };
  const openEdit = (row) => { setForm({ name: row.name || row.className || '', authorizedShares: row.authorizedShares || '', pricePerShare: row.pricePerShare || '', type: row.type || 'common' }); setMutationError(null); setModal({ open: true, editing: row }); };
  ...
  return (
    <div>
      ...
      <Modal isOpen={modal.open} onClose={() => { setModal({ open: false, editing: null }); setMutationError(null); }} title={modal.editing ? 'Edit Share Class' : 'Add Share Class'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium mb-1">Authorized Shares</label><input required type="number" min="0" value={form.authorizedShares} onChange={(e) => setForm({ ...form, authorizedShares: e.target.value })} className="w-full px-3 py-2 border rounded-md" /></div>
          <div><label className="block text-sm font-medium mb-1">Price per Share</label><input type="number" step="0.01" min="0" value={form.pricePerShare} onChange={(e) => setForm({ ...form, pricePerShare: e.target.value })} className="w-full px-3 py-2 border rounded-md" /></div>
      ...
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={() => deleteMut.mutate(deleteId)} title="Delete Share Class" message="Are you sure you want to delete this share class?" />
    </div>
  );
}
