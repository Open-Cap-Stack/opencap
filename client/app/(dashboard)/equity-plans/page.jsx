...
export default function EquityPlansPage() {
  ...
  const createMut = useMutation({ mutationFn: (d) => equityPlanService.createEquityPlan(d), onSuccess: () => { setMutationError(null); qc.invalidateQueries({ queryKey: ['equityPlans'] }); setModal({ open: false, editing: null }); }, onError: (err) => { setMutationError(err.response?.data?.message || 'Failed to create equity plan'); } });
  const updateMut = useMutation({ mutationFn: ({ id, ...d }) => equityPlanService.updateEquityPlan(id, d), onSuccess: () => { setMutationError(null); qc.invalidateQueries({ queryKey: ['equityPlans'] }); setModal({ open: false, editing: null }); }, onError: (err) => { setMutationError(err.response?.data?.message || 'Failed to update equity plan'); } });
  const deleteMut = useMutation({ mutationFn: (id) => equityPlanService.deleteEquityPlan(id), onSuccess: () => { qc.invalidateQueries({ queryKey: ['equityPlans'] }); setDeleteId(null); }, onError: (err) => { alert(err.response?.data?.message || 'Failed to delete equity plan'); } });

  const openCreate = () => { setForm(emptyForm); setMutationError(null); setModal({ open: true, editing: null }); };
  const openEdit = (row) => { setForm({ name: row.name || '', type: row.type || 'stock_option', totalShares: row.totalShares || '', status: row.status || 'active' }); setMutationError(null); setModal({ open: true, editing: row }); };
  ...
  return (
    <div>
      ...
      <Modal isOpen={modal.open} onClose={() => { setModal({ open: false, editing: null }); setMutationError(null); }} title={modal.editing ? 'Edit Equity Plan' : 'Add Equity Plan'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium mb-1">Total Shares</label><input required type="number" min="0" value={form.totalShares} onChange={(e) => setForm({ ...form, totalShares: e.target.value })} className="w-full px-3 py-2 border rounded-md" /></div>
