...
export default function StakeholdersPage() {
  ...
  const createMut = useMutation({ mutationFn: (d) => stakeholderService.createStakeholder(d), onSuccess: () => { qc.invalidateQueries({ queryKey: ['stakeholders'] }); setModal({ open: false, editing: null }); } });
  const updateMut = useMutation({ mutationFn: ({ id, ...d }) => stakeholderService.updateStakeholder(id, d), onSuccess: () => { qc.invalidateQueries({ queryKey: ['stakeholders'] }); setModal({ open: false, editing: null }); } });
  const deleteMut = useMutation({ mutationFn: (id) => stakeholderService.deleteStakeholder(id), onSuccess: () => { qc.invalidateQueries({ queryKey: ['stakeholders'] }); setDeleteId(null); } });

  const openCreate = () => { setForm(emptyForm); setModal({ open: true, editing: null }); };
  const openEdit = (row) => { setForm({ name: row.name || '', email: row.email || '', role: row.role || '', ownershipPercentage: row.ownershipPercentage || '' }); setModal({ open: true, editing: row }); };
  ...
  return (
    <div>
      ...
      <Modal isOpen={modal.open} onClose={() => setModal({ open: false, editing: null })} title={modal.editing ? 'Edit Stakeholder' : 'Add Stakeholder'}>
